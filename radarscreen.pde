...
void setup() {
  ...
  orcFont = createFont(PFont.list()[0], 14);
  noLoop();
}
...
void drawObject() {
  ...
  pixsDistance = float(iDistance)*900.0/float(MAX_RANGE); // covers the distance from the sensor from mm to pixels
  // limiting the range to MAX_RANGE mm
  if (iDistance<MAX_RANGE) {
...
void drawText() { // draws the texts on the screen
  ...
  fill(98, 245, 31);
  textSize(25);
  text(MAX_RANGE/4 + "mm", 1180, 990);
  text(MAX_RANGE/2 + "mm", 1380, 990);
  text(MAX_RANGE*3/4 + "mm", 1580, 990);
  text(MAX_RANGE + "mm", 1780, 990);
  textSize(40);
  text("Object: " + noObject, 240, 1050);
  text("Angle: " + iAngle +" °", 1050, 1050);
  text("Distance: ", 1380, 1050);
  if (iDistance<MAX_RANGE) {
    text("        " + iDistance +" mm", 1450, 1050);
  }
  textSize(25);
  fill(98, 245, 60);
